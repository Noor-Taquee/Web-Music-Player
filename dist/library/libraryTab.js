...
function openPlaylist(playlist) {
    ...
    if (playlist.songs.length > 0) {
        for (let sng of playlist.songs) {
            playlistPanelSongDiv.appendChild(createSongBtn(sng, songData[sng].name, songData[sng].artist, songData[sng].image, "rectangle"));
        }
    }
    else {
...
function showDeletePlaylistPanel() {
    hidePlaylistOptions();
    if (libraryPanel_content.contains(confirmDelPlaylistPanel))
        return;
    confirmDelPlaylistPanel = createConfirmationPanel("Delete playlist?", `Are you sure you want to delete "${playlistInFocus.name}"`, "Delete", () => { deletePlaylist(playlistInFocus); }, hideDeletePlaylistPanel);
