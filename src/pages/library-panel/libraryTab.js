...
function createPlaylist(playlist) {
  ...
  const playlistInfo = createElement("div", {
    className: "playlist-btn-info-div",
  });
  ...
  if (playlist.image.length > 0) {
    ...
    playlistPic.appendChild(
      createElement("i", { className: "ph-bold ph-music" }),
    );
  }

  // about the playlist
  const aboutPlaylistDiv = createElement("div", {
    className: "playlist-btn-about-div",
  });
  ...
  aboutPlaylistDiv.appendChild(
    createElement("p", {
      className: "playlist-btn-about",
      textContent: `${playlist.privacy} | ${playlist.songs.length} songs`,
    }),
  );

  // options button for playlist
  const bnOptions = createElement(
    "button",
    { className: "playlist-options-btn", onclick: null },
...
libraryPanel_header.appendChild(
  createElement("p", {
    className: "library-title",
    textContent: "My Playlists",
  }),
);
...
libraryPanel_header.appendChild(
  createElement(
    "button",
    { id: "add-playlist", className: "toggle", onclick: showAddPlaylistPanel },
...
const libraryContentText = createElement("p", {
  className: "div-text",
...
const playlistPanel = createElement("div", {
  className: "playlist-panel content-fill",
});
...
playlistPanel_header.appendChild(
  createElement(
    "button",
    { className: "back-btn toggle", onclick: f_backPlaylistPanel },
    [createElement("i", { className: "ph-bold ph-arrow-left" })],
  ),
);

const playlistPanel_content = createElement("div", {
  className: "playlist-panel-content",
});
...
function openPlaylist(playlist) {
  ...
  const playlistInfoPanel = createElement("div", {
    className: "playlist-info-panel",
  });
  playlistPanel_content.appendChild(playlistInfoPanel);

  // Playlist image
  const playlistPanelImage = createElement("p", {
    className: "playlist-panel-image",
    textContent: "",
  });
  ...
  const playlistInfoPanelDetails = createElement("div", {
    className: "playlist-info-panel-details",
  });
  playlistInfoPanel.appendChild(playlistInfoPanelDetails);

  // Playlist Name
  playlistInfoPanelDetails.appendChild(
    createElement("p", {
      className: "playlist-panel-title",
      textContent: playlist.name,
    }),
  );
  playlistInfoPanelDetails.appendChild(
    createElement("p", {
      className: "playlist-panel-info",
      textContent: `${playlist.privacy}`,
    }),
  );
  playlistInfoPanelDetails.appendChild(
    createElement("p", {
      className: "playlist-panel-info",
      textContent: `${playlist.songs.length} songs`,
    }),
  );
  ...
  playlistInfoPanelDetails.appendChild(
    createElement(
      "button",
      { id: "edit-playlist", className: "edit-btn", onclick: null },
      [
        createElement("i", { className: "ph-fill ph-pencil" }),
        createElement("p", { textContent: "Edit" }),
      ],
    ),
  );

  // Area for songs in the playlist
  const playlistPanelSongDiv = createElement("div", {
    className: "playlist-panel-song-div",
  });
...
function f_confirmAddPlaylist() {
  ...
  if (nameOfPlaylist.trim().length < 1) {
...
const addPlaylistPanel = createElement("div", {
...
const addPlaylistForm = createElement("div", {
  className: "add-playlist-form",
});
addPlaylistPanel.appendChild(addPlaylistForm);

const playlistNameInput = createElement("input", {
  type: "text",
  placeholder: "Name",
});
addPlaylistForm.appendChild(
  createElement("div", { className: "playlist-form-input-div" }, [
    createElement("i", { className: "ph-bold ph-music-notes-simple" }),
    playlistNameInput,
  ]),
);

const playlistImageInput = createElement("input", {
  type: "text",
  placeholder: "Image Link",
});
addPlaylistForm.appendChild(
  createElement("div", { className: "playlist-form-input-div" }, [
    createElement("i", { className: "ph-bold ph-link" }),
    playlistImageInput,
  ]),
);

const addPlaylistBtnDiv = createElement("div", {
  className: "playlist-form-btn-div",
});
addPlaylistPanel.appendChild(addPlaylistBtnDiv);

const bnCancelAddPlaylist = createElement(
  "button",
  { className: "playlist-form-btn cancel", onclick: hideAddPlaylistPanel },
  [createElement("p", { textContent: "cancel" })],
);
addPlaylistBtnDiv.appendChild(bnCancelAddPlaylist);

const bnConfirmAddPlaylist = createElement(
  "button",
  { className: "playlist-form-btn", onclick: f_confirmAddPlaylist },
  [createElement("p", { textContent: "Create" })],
);
addPlaylistBtnDiv.appendChild(bnConfirmAddPlaylist);

const bnConfirmAddPlaylistCon = createElement("div", {
  className: "playlist-btn-loader",
});
bnConfirmAddPlaylistCon.appendChild(
  createElement("i", { className: "ph-bold ph-circle-notch" }),
);
...
function showDeletePlaylistPanel() {
  ...
  confirmDelPlaylistPanel.addEventListener("animationend", () => {
    confirmDelPlaylistPanel.style.animation = "none";
  });
...
function hideDeletePlaylistPanel() {
  ...
  confirmDelPlaylistPanel.addEventListener(
    "animationend",
    () => {
...
function deletePlaylist(playlist) {
  currentUser.playlistList.splice(
    currentUser.playlistList.indexOf(playlist),
    1,
  );
  currentUser.sync();
  loadPlaylists();
...
optionPanel.appendChild(
  createElement(
    "button",
    { className: "option-panel-btn .cancel", onclick: hidePlaylistOptions },
...
optionPanel.appendChild(
  createElement(
    "button",
    { className: "option-panel-btn", onclick: showRenamePlaylistPanel },
    [
      createElement("i", { className: "ph-bold ph-note-pencil" }),
      createElement("p", { textContent: "Rename" }),
    ],
  ),
);
...
optionPanel.appendChild(
  createElement(
    "button",
    { className: "option-panel-btn", onclick: showDeletePlaylistPanel },
    [
      createElement("i", { className: "ph-bold ph-trash" }),
      createElement("p", { textContent: "Delete" }),
    ],
  ),
);
