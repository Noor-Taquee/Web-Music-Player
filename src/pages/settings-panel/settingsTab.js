...
function setTheme(theme) { 
  ...
  updateDeviceColor();
  if (signedIn) { currentUser.theme = theme; currentUser.sync(); }
}
...
function openImportedPanel(id, openFunction, path, userCheck = false) {
  if (userCheck && !signedIn) { alert("You need to be logged in to use this feature!"); return; }

  if (document.getElementById(id)) { openFunction(); return; }
