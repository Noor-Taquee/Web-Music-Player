...
function createSettingsPanelBtn(
  ...
  iconDiv.appendChild(
    createElement("i", { className: "ph-bold ph-" + iconName }),
  );
  ...
  con.appendChild(
    createElement("p", { className: "button-header", textContent: buttonText }),
  );
  con.appendChild(
    createElement("p", {
      className: "button-description",
      textContent: buttonDescription,
    }),
  );
...
accountPanel_header.appendChild(
  createElement("p", {
...
const bnLogin = createElement(
  "button",
  {
    className: "login-btn",
    onclick: () => {
      openImportedPanel("login-js", openLoginPanel, "src/login/loginTab.js");
    },
  },
...
const profileInfoCard = createElement("div", {
...
function showInfo() {
  if (signedIn) {
    ...
    if (accountPanel_content.contains(accountInfoDiv)) return;
    accountPanel_content.replaceChild(accountInfoDiv, bnLogin);
  } else {
    profileName.textContent = "";

    if (accountPanel_content.contains(bnLogin)) return;
